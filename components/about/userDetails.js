  ...
  if (user && user.userExp) {
    ...
    empView = empResult.map((item) => {
      ...
      return (
        <li className="list-group-item" key={item.expID}>
          <h5 className="card-title">{item.expDesignation} at {item.comName}</h5>
          <p className="text-muted"> {expStart} to {expEnd}</p>
    ...
    if (user.userSkill.length !== 0) {
      skillView = user.userSkill.map((item) => {
        return (
          <span key={item.skiID}>{item.skillName}, </span>
        );
      }
      );
    }
    if (user.userIndustry.length !== 0) {
      indView = user.userIndustry.map((item) => {
        return (
          <span key={item.IndID}>{item.IndName}, </span>
        );
      }
      );
    }
    if (user.userLanguages.length !== 0) {
      langView = user.userLanguages.map((item) => {
        return (
          <span key={item.lanID}>{item.langName}, </span>
        );
      }
      );
    }
      ...
      <div className="card mb-1">
        <div className="card-header text-center">
          ...
          {/* <div className="row">
            ...
            {user && user.userInfo && user.userInfo.userAddress && <div className="rows">
              <p className="text-left"><b>Location :</b> {user && user.userInfo && user.userInfo.cityName}, {user && user.userInfo && user.userInfo.countryCode}</p>
            </div>}
            </div>
            {user && user.userInfo && user.userInfo.userAbout && <div className="card">
            <div className="card-body">
              <div className="card-text">{user && user.userInfo && user.userInfo.userAbout}</div>
            </div>
          </div>}

          {skillView && <div className="card-body">
            {skillView && <p className="card-text"><b>Skill :</b> {skillView} </p>}
            {indView && <p className="card-text"><b>Industry :</b> {indView} </p>}
            {langView && <p className="card-text"><b>Language :</b> {langView}</p>}
          </div>}
